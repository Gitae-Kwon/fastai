...
if st.button("🟢 매핑 실행"):
    ...
    # 빈 시리즈를 만들고, 매핑명==정제상품명인 곳에만 정산서_콘텐츠명 복사
    channel = pd.Series("", index=result.index)
    mask = result["매핑_콘텐츠마스터명"] == result["정제_상품명"]
    channel.loc[mask] = result.loc[mask, "정산서_콘텐츠명"]

    result.insert(pos, "채널_콘텐츠명", channel)
